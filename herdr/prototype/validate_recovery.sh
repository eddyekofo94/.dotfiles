...
record result "$(jq -cn --arg version "$("$herdr" --version)" '{status:"PASS",version:$version,session:"rc",snapshot_restored:true,pane_history_replayed:true,native_agent_resumed:true,arbitrary_process_resume:false,production_configuration_modified:false,migration_authorized:false}')"

...
| Seamless Neovim-to-multiplexer edge navigation | **Custom implementation** | Herdr has direct modified-chord command bindings, pane foreground-process inspection, key injection, and directional focus. The official docs do not ship a Vim navigator, but those primitives support an adapter. [Configuration](https://herdr.dev/docs/configuration/#custom-command-keybindings), [CLI reference](https://herdr.dev/docs/cli-reference/) |
...
| Golden-ratio autoresize on pane focus | **Custom implementation** | `pane.focused` is a subscribable/plugin-hook event, while the CLI exposes layout snapshots and fractional directional resize. No built-in golden-ratio focus policy is documented. [Socket API](https://herdr.dev/docs/socket-api/), [CLI reference](https://herdr.dev/docs/cli-reference/) |
| Animated or pixel-smooth Herdr scrollback | **No documented equivalent** | Herdr exposes `ui.mouse_scroll_lines` as a positive integer and reports line-based scroll state/actions; no easing, duration, animation, or pixel-scroll configuration is documented. Neovim plugins can still animate scrolling inside Neovim. [Config reference](https://herdr.dev/docs/config-reference/), [Socket API](https://herdr.dev/docs/socket-api/) |

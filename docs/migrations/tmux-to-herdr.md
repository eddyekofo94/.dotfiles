...
| `prefix+N`, `prefix+Ctrl-n` | New tab after current | approved native `prefix+c`, plus the direct `Alt-Ctrl-t` chord | [x] Real `prefix+c` input created tabs `w1:t2`, `w1:t3`, and `w1:t4`, each with one pane and its own live sentinel PID; legacy aliases remain intentionally removed. Kitty CSI-u `116;7u` creates exactly one focused cwd-following tab. |
| `prefix+t/T` | Next/previous tab | approved native `prefix+n/p`, plus the direct `Alt-Ctrl-n`/`Alt-Ctrl-p` chords | [x] Real `prefix+n/p` input wrapped `w1:t4 -> w1:t1 -> w1:t4`; legacy aliases remain intentionally removed. Kitty CSI-u `110;7u`/`112;7u` step the same tab cycle. |

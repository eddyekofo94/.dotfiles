...
# XcodeBuildMCP creates a Unix-domain socket below this runtime. A named
# worktree path can exceed sockaddr_un's macOS limit before the socket name is
# appended, so keep the disposable verification root deliberately short.
runtime=$(mktemp -d /private/tmp/pi-pilot-verify.XXXXXX)
...
node --no-warnings "$pi_dir/tests/herdr_agent_state_test.mjs"
...
# settings.json intentionally names the live repository path. A linked
# worktree must exercise its own extension source or the full gate can pass
# while testing yesterday's implementation.
verified_settings="$runtime/settings.json"
jq --arg extension "$pi_dir/extensions/eddy-compat.ts" \
  '.extensions[0] = $extension' "$pi_dir/settings.json" >"$verified_settings"
